...
def get_reachable(bmatrix: RSMMatrixSparse) -> Set[Tuple[int, int]]:
    """
    ...
    for state_from, state_to in zip(*tc.nonzero()):
        if (
            ...
            result.add(
                (
                    state_from // len(bmatrix.indexed_states),
                    state_to // len(bmatrix.indexed_states),
                )
            )
...
def rpq(
    ...
    return get_reachable(bmatrix=intersection)
